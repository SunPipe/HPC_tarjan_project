 ...
 * 
 *  This file is the MPI version of Tarjan algorithm to find strongly connected components
 * 
 ...
 * It performs some additional logic to handle special cases where the vertex is a supernode and its name is not -1
...
void scc(int v, vertex *graph, TStack *stack, TStack *temp, int *curr_index)
{
    int i, c, n;
    ...
    if (graph[v].supernodo == 0) // if the vertex has the flag supernodo equals to 1, it's ignored by the algorithm
    {
                                                   ...
                                                   * until it reaches the current vertex. Then it add a -1 value to the stack as a convention to ensure division between the SCCs founded.*/
        {
            ...
            if (graph[n].nome != -1)
            {
                if (graph[n].index == -1)
                {
                    scc(n, graph, stack, temp, curr_index);
 ...
 * adiacency list combinating the ones of the "children" nodes and adding it on the hashtable. Moreover is updated, for the vertex the compose that SCC, the adjacency list, cleaning it
...
int updateGraph(vertex *graph, int *sccFounded, int num, THT *hashtable, int numberOfVertices, int rank, int iterazione, int size, int initialVertices)
{
    ...
    // For a clear explanation it's suggested to first read the else branch and the relative comments, and after the if branch
    for (int j = 0; j < num; j++)
    {
        if (sccFounded[j] == -1)
        {
 ...
 * the array), it's setted the array value to 5 so in next calls to this function they will not be erroneously matched with other processes*/
    ...
    else
    {
        if (rank == 0) // The MPI process with rank 0 is the scheduler and creator of the graph (if the program is start with just one MPI process, this rank it's not used as a scheduler but as a SCC founder)
        {
            ...
            else // here it's displayed the version with just one MPI process (so SCC founder job)
            {
                ...
                graph = (vertex *)malloc((numberOfVertices+(numberOfVertices/2)) * sizeof(vertex)); // the graph is allocated as the numberOfVertices * size of the vertex struct plus a bigger space to store new SCC founded (max 1/2 of the total vertices)
                ...
                for (int j = 0; j < numberOfVertices; j++)               // this for loop is used to find the SCCs of the graph, it calls the Tarjan algorithm from each vertex if that vertex is
                ...
                numberOfVertices += updateGraph(graph, temp.a.items, dim, hashtable, numberOfVertices, 1, 0, 1, numberOfVertices); // it's called the function that
                ...
                FILE *fp;
                char path[200];
                sprintf(path, "ResultSCC/opt%d/type%d/MpiOnly_%d_%d_%d.txt", atoi(argv[5]), atoi(argv[3]), numberOriginario, edges, size); // define the path where to store the SCCs
                ...
                for (int vertexCounter = 0; vertexCounter < numberOfVertices; vertexCounter++) // in this for loop it's saved on file the SCCs of the total graph and contextually it's freed
                ...
                fprintf(fp, "MPIOnly;0;%d; %ld.%06ld; %ld.%06ld; %ld.%06ld; %ld.%06ld;\n", size, (long int)total_time.tv_sec, (long int)total_time.tv_usec, (long int)graph_creation_time.tv_sec, (long int)graph_creation_time.tv_usec, (long int)communication_time.tv_sec, (long int)communication_time.tv_usec, (long int)tarjan_time.tv_sec, (long int)tarjan_time.tv_usec);
                fclose(fp);
            }
        ...
        else // here it starts the slaves job
        {
            struct timeval all_end, start, end, total_time, graph_creation_time, communication_time, tarjan_time; // variables used for measure time of the steps of the algorithm
            ...
            vertex *graph;
            int actualLength = newNumberOfVertices + newNumberOfVertices / 2 * (login2(size - 1) + 1); // the number of vertices of the graph is a bit more greater than the minimum one, because
            ...
            while (i < iterations) // here starts the algorithm, just one slave will reach the end of this while, other will die previously
            {
                ...
                privateNumberOfVertices += updateGraph(graph, temp.a.items, dim, hashtable, actualLength, indice, i, slaves, newNumberOfVertices); // it's called the function that
                // update the graph adding the new vertex founded by the algorithm
            ...
            if (i == iterations) // at the end of the loop, if this rank have reached the last iteration, it means that he his the rank 1, and he have to compute the final result
            {
                ...
                FILE *fp;
                char path[200];
                sprintf(path, "ResultSCC/opt%d/type%d/MpiOnly_%d_%d_%d.txt", atoi(argv[5]), atoi(argv[3]), numberOfVertices, edges, size); // define the path where to store the SCCs
            ...
            if (i == iterations) // the rank who survives (rank 1) have to write on the file all the times he has calculated: total_time, graph_creation_time,communication_time and tarjan_time
            {
                ...
                fprintf(fp, "MPIOnly;0;%d; %ld.%06ld; %ld.%06ld; %Lf; %Lf;\n", size, (long int)total_time.tv_sec, (long int)total_time.tv_usec, (long int)graph_creation_time.tv_sec, (long int)graph_creation_time.tv_usec, communication_time2, tarjan_time2);
                fclose(fp);
            }
            MPI_Finalize(); // terminates MPI execution environment
            exit(0);
